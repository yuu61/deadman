...
func (m Model) pingOne(idx, gen int) tea.Cmd {
	...
	t := m.rows[idx].Target
	return func() tea.Msg {
		return pingResultMsg{idx: idx, gen: gen, target: t, res: t.Pinger.Send(context.Background())}
	}
}
...
func (m Model) startSyncRound() (tea.Model, tea.Cmd) {
	...
	if idx < 0 {
		return m, tea.Tick(allTargetInterval, func(time.Time) tea.Msg { return roundStartMsg{gen: gen} })
	}
...
func (m Model) advanceSync(cur int) tea.Cmd {
	...
	if next < 0 {
		return tea.Tick(allTargetInterval, func(time.Time) tea.Msg { return roundStartMsg{gen: gen} })
	}
	return tea.Tick(pingInterval, func(time.Time) tea.Msg { return pingStartMsg{idx: next, gen: gen} })
}
