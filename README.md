...
`scale` はコマンドラインの `-s` / `--scale` と同じ設定で、**明示した `-s` が `scale` ディレクティブより優先**されます（どちらも無ければ既定 10。小数値も指定でき、サブミリ秒のスケールに対応します）。実用範囲（おおよそ 0.0001〜1000000 ms）を外れた極端な値・不正値（0・負数・`inf`・`nan`）は無視され、既定 10 に戻ります。
