...
- `log` モード … `l` キーで RTT スケールの**対数表示**を切り替えます（linear → 底 e → 底 e²）。
  ...
  対数モードでは `↑`/`↓` で設定するスケール値が**対数窓の左端（floor）**として働き、係数（底）で窓の広さが決まります（底 e で 7 段が約 2.6 桁、底 e² で約 5.2 桁）。
...
`scale` はコマンドラインの `-s` / `--scale` と同じ設定で、**明示した `-s` が `scale` ディレクティブより優先**されます（どちらも無ければ既定 10。小数値も指定でき、サブミリ秒のスケールに対応します）。
