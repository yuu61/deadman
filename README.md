...
p           統計値の表示精度を切り替える（整数 ms → 小数 1 桁 → μs）
...
precision ms.1      統計値の表示精度（ms / ms.1 / us のいずれか。既定 ms）
```
...
- `precision` … 統計列（RTT / AVG / MIN / MAX / JIT）の数値表記です。`ms`（整数）/ `ms.1`（小数 1 桁）/ `us`（マイクロ秒）から選べ、実行中は `p` キーで循環します。
