...
- **日付**: 2026-06-10（調査） / 2026-06-12 実装済み項目を除去
- **対象コミット**: `dedae0e`（調査時 master）
- **調査方法**: 6 次元の並列調査（プロービング層 / TUI / 設定・CLI / 統計・ログ / 参照ツール比較 / コード品質）→ 重複排除 → 各候補を保守者視点で敵対的検証。すべて実コード接地（`file:line`）で確認。
- **集計**: 候補 76 件 → ユニーク 52 件 → 採用 16 件 / 却下 35 件。うち **3 件（ビューポート/スクロール・resolve_family・RTT 列あふれ）は実装完了済みのため本書から除外**し、未着手の 13 件を以下に記す。
...
### 1. per-target の `timeout=` 属性 〔軸1 / additive / S / なし(+Key)〕
...
### 2. 行フィルタ（Down のみ表示） 〔軸2 / additive / S / なし〕

列可視性制御（`columns` ディレクティブ + `v`/`m` キー、`columns.go` の visible マップ）の**行版**。100+ ホスト時の認識負荷を下げる。実装済みのビューポート/スクロールと組むと効果が大きい。
...
### 3. TCP/hping3 の flags・source-port 拡張 〔軸1 / additive / S / なし〕
...
| --- | --- | --- | --- | --- | --- | --- |
| 4 | **DSCP/ToS マーキング** (`dscp=EF`) | 軸1 | new | additive | M | QoS 経路検証（`ping -Q`/`fping -O` 相当）。直接ICMP は `SetTrafficClass`、nexthop は手組ヘッダの ToS。`EF/AF41/CS5/46` を解決する小テーブル（`precisionModes` 同様の単一ソース） |
| 5 | **ソート** (loss/state/name 順) | 軸2 | new | additive | M | `displayOrder []int` 間接層 + `slices.SortStableFunc`。Sep をアンカーに各セグメント内のみ並べ替え。probe 側（`commands.go`）は config 順固定で矢印ズレ回避。**live-rtt は除外**（毎秒ジャンプして追跡を壊す） |
| 6 | **Down 継続時間列 `SINCE`** | 軸2 | new | additive | M | `Target.LastStateChange` を遷移時のみ記録 → `DOWN 3m` 表示。`statColumns` に固定列 1 エントリ。NOC トリアージ用 |
| 7 | **直近 N 窓の loss 率列** | 軸2 | new | additive | M | lifetime `LossRate`（`target.go:97`）は長時間運用で老化。`WindowLossRate(n)` + 既定 OFF 列。任意で `window` ディレクティブ（`scaleDirective` 同型）。RESULT バーの定量化 |
| 8 | **`--check`/`--validate` フラグ** | 軸CLI | new | additive | S | パースして警告だけ出して終了（CI/投入前の typo・dropped token 検出）。`startupWarnings`（`parse_warn.go`）を export ラップして `main()` から呼ぶ。決定的な parse 警告が主目的（nexthop 系は環境依存） |
| 9 | **TTL 列** | 軸2 | new | additive | S | `Target.TTL`（`target.go:57,90`）は捕捉済み・未表示。`statColumns` に固定列 1 エントリ。**observ 可は直接 ICMP/RouterOS のみ**で他モードは `-1` → 空白/`-` にマップする専用 Cell が必須。最も安いが採用群で価値は最低 |
...
| # | 提案 | 軸 | 種別 | effort | 要点 |
| --- | --- | --- | --- | --- | --- |
| 10 | **async ラウンド中リロードの inflight/pending 一貫性テスト** | code-quality | test | S | `handlePingResult` の gen ガード（`model.go:343`）が `pending--`（`model.go:363`）より前にある順序を回帰固定。リファクタで順序が崩れると新ラウンドの pending が早期に `<=0` になり `scheduleNextRound` 二重発火するが、現状それを捕まえるテストが無い |
| 11 | **SNMP v3 認証/暗号化対応** | 軸1 | improve | M | `snmp_version`/`snmp_user`/`snmp_auth`/`snmp_priv` 属性。ニッチの中のニッチ + テスト基盤新設が要るため優先度低 |
| 12 | **未知 `columns` キーの警告 + サジェスト** | 軸CLI | improve | M | `applyColumn`（`config.go:233`）/`buildVisible`（`columns.go:157`）が registry 外キーを黙って捨てる。`columnWarnings` + 距離 ≤2 の Levenshtein 近傍提示。relay 属性 typo は既に Dropped 警告で表面化済みのため scope 外 |
| 13 | **historyCap 境界動作テスト** | code-quality | test | S | `historyCap=256`（`target.go:35`）の超過 drop と truncate 後 newest-first 順を未検証（既存テストは最大 3 エントリ）。将来の ring buffer 化の安全網 |
...
軸2 の「窓化」はビューポート/スクロール（実装済み）と段組み表示で一段進んだ。残る伸び代は「軸1 の深掘り」と、窓化を補完する行レベルの絞り込み。一押しは:

- **#2 行フィルタ** — 実装済みのビューポートと組み、大規模監視の認識負荷を下げる。
- **#1 timeout + #3 TCP flags** — いずれも effort S で軸1 を素直に伸ばす。

最小で型を見るなら **#2 行フィルタ** か **#1 timeout** が着手しやすい。すべての変更で `make check` を通し、契約（設定構文/CLI）に触れるものは README を同時更新すること。
