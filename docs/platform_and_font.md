...
成功したプローブは、RESULT バーの段階に応じて色分けされます。段階は文字の高さや数字と同じもので、`scale`・対数表示・表示文字（`glyph`）によって決まります。最も速い段階が緑、中央の段階が黄、最も遅い段階（`█` / `@` / `9`）が赤で、その間は段階ごとに少しずつ変わります。`scale` を変えると色の境目も変わります。失敗（`X` / `t` / `s`）は従来どおり端末の赤です。
